...
const API_URL = API_ENDPOINTS.VISITORS;

const SUGGESTIONS_API_URL = API_ENDPOINTS.VISITORS; // For physical device on same WiFi



// Test the API connection

const TEST_API_URL = API_ENDPOINTS.VISITORS;
...
export default function VisitorFormScreen({ navigation }: VisitorFormScreenProps) {
  ...
  const getUserToken = async () => {
    ...
    try {
      ...
      const storedToken = await Keychain.getGenericPassword();

      if (storedToken) {

        console.log('Retrieved user token:', storedToken.password?.substring(0, 20) + '...');
  ...
  useEffect(() => {
    ...
    const fetchToken = async () => {
      ...
      console.log('Retrieved token:', storedToken ? 'exists' : 'null');
  ...
  const fetchPhoneSuggestions = async (query: string) => {
    ...
    if (!query || query.length < 5) {
      ...
      console.log('Query too short (< 5 digits), clearing suggestions');
  ...
  const handleSubmit = async () => {
    ...
    console.log('Current user from route:', user);
    ...
    // Try to get fresh token if currentToken is null

    let finalToken = currentToken;

    if (!currentToken) {

      console.log('No token in state, trying to fetch from storage...');

      const freshToken = await getUserToken();

      console.log('Fresh token retrieved:', freshToken ? 'yes' : 'no');
    ...
    if (!finalToken) {
      ...
      console.log('No token available - using temporary bypass for testing');

      // Temporary bypass until Render deployment is complete

      // showAlert('🔐 Authentication Required', 'Please login again to get a valid token', 'error');
    ...
    console.log('Using token for API call:', finalToken ? finalToken.substring(0, 20) + '...' : 'no token');
  ...
  try {
    ...
    const leadData = {
      ...
      company_id: user.company_id, // Add company_id from user data
